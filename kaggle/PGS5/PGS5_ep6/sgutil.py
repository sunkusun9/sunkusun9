...
class SGCache:
    def __init__(self, img_path, result_path, model_path):
        ...
        for i in [img_path, result_path, model_path]:
    ...
    def read_prd(self, cv_name, index = None, columns = None):
        ...
        if len(prd.shape) == 1:
            ...
            return pd.DataFrame(
                prd, index = index, columns = columns
            ) if index is not None else prd

    def read_prds(self, cv_names, index = None, columns = None):
        prds = np.stack([
            joblib.load(os.path.join(self.result_path, cv_name + '.prd')) for cv_name in cv_names
        ], axis=1)
        if prds.shape[-1] == len(cv_names):
            return pd.DataFrame(
                prds, index = index, columns = cv_names
            ) if index is not None else prds
        else:
            return pd.DataFrame(
                prds, index = index, columns = pd.from_product(cv_names, columns)
            ) if index is not None else prds
